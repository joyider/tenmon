# -*- coding: utf-8 -*-
# Filename: helpers by: andrek
# Timesamp:2018-10-30 :: 21:01 using PyCharm
...
def single_task_runner():
    def task_exc(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            lock_id = "tenforward-single-exm-task-" + func.__name__
            acquire_lock = lambda: di.add_variable(lock_id, "true")
            release_lock = lambda: di.remove_variable(lock_id)
            if acquire_lock():
                try:
                    func(*args, **kwargs)
                finally:
